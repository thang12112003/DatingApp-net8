...
public async Task<PagedList<MemberDto>> GetUserLikes(LikesParams likesParams)
{
    ...
    {
        case "liked": // Những user mà mình đã like
            query = likes
        ...
        case "likedBy": // Những user đã like mình
                query = likes
                .Where(x => x.TargetUserId == likesParams.UserId)
                .Select(x => x.SourceUser) // ✅ Đổi từ SourceUserId -> SourceUser (AppUser)
                .ProjectTo<MemberDto>(mapper.ConfigurationProvider);
                break;

        default: // Những user đã like nhau
